...
export function ConsistencyWorkbench({
  projectId,
  initialIssues,
  source,
  loadError,
  ...
  const [standardValue, setStandardValue] = useState(
    initialIssues[0]?.suggestedValue ?? "",
  );
  ...
  const filtered = useMemo(
    () =>
      issues.filter(
        (item) =>
          `${item.field}${item.reason}${item.owner}`
            .toLowerCase()
  ...
  function selectIssue(issue: ConsistencyIssue) {
    setSelectedId(issue.id);
    setStandardValue(issue.suggestedValue);
    setReason("");
  }
  async function resolve(mode: "standard" | "reasonable") {
    if (!reason.trim()) {
      setFeedback({
        ...
        status: "error",
        title: "缺少处理原因",
        message: "请先填写处理原因，所有一致性决策都必须可审计。",
      });
      return;
    }
    ...
    const resolution =
      mode === "standard"
        ? `采用标准值：${standardValue}。${reason}`
        : `接受合理差异：${reason}`;
    const result = await phaseApi.resolveConsistency(
      selected.id,
      status,
      resolution,
    );
    if (result.failed) {
      setFeedback(toFeedback(result, source, "一致性问题处理失败"));
      return;
    }
    setIssues((current) =>
      current.map((item) =>
        item.id === selected.id
          ? {
              ...item,
              suggestedValue: standardValue,
              status: mode === "standard" ? "resolved" : "reasonable",
              reason: resolution,
            }
          : item,
      ),
    );
    setFeedback(toFeedback(result, source, "一致性问题已处理"));
  }
  async function createTask() {
    const result = await phaseApi.createTask(
      projectId,
      {
        title: `整改一致性问题：${selected.field}`,
        priority: selected.risk === "fatal" ? "critical" : "high",
        status: "todo",
        sourceType: "consistency",
        sourceLabel: `${selected.field} · ${selected.id}`,
        reason: selected.reason,
        evidence: selected.sources
          .map((item) => `${item.document} 第 ${item.page} 页`)
          .join("；"),
      },
      selected.id,
    );
    ...
    <div className="page consistency-page">
      <header className="page-header">
        <div className="page-title-group">
          <h1>一致性检查</h1>
          <p>
            确定性比较名称、日期、金额、人员和承诺；发现差异后由人工选择标准值。
          </p>
        </div>
      ...
      <section className="consistency-stats">
        <Stat
          icon={<CircleDollarSign />}
          label="金额差异"
          value={issues.filter((item) => item.type === "amount").length}
          tone="fatal"
        ...
        <Stat
          ...
          label="主体 / 人员"
          value={
            issues.filter((item) => ["entity", "person"].includes(item.type))
              .length
          }
        ...
        <Stat
          ...
          label="承诺差异"
          value={issues.filter((item) => item.type === "commitment").length}
        ...
        <Stat
          ...
          value={
            issues.filter((item) =>
              ["resolved", "reasonable"].includes(item.status),
            ).length
          }
          tone="success"
          onClick={() => setType("all")}
      ...
      <section className="workbench-alert" role="note">
        <GitCompareArrows size={15} />
        <span>
          <strong>
            {
              issues.filter((item) => ["open", "review"].includes(item.status))
                .length
            }{" "}
            个未解决问题
          </strong>{" "}
          · 共比较 {issues.reduce((sum, item) => sum + item.documents, 0)}{" "}
          个文档来源、
          {issues.reduce((sum, item) => sum + item.discoveredValues, 0)}{" "}
          个发现值；所有标准值均需人工决定。
        </span>
      </section>
      <MutationFeedback result={feedback} />
      <div className="consistency-layout">
        <section className="panel consistency-list">
            ...
            <table className="consistency-table">
              ...
              <tbody>
                {filtered.map((issue) => (
                  <tr
                    key={issue.id}
                    className={issue.id === selected.id ? "selected" : ""}
                  ...
                  >
                    ...
                    <td>
                      <strong>{issue.discoveredValues}</strong>
                      <small>{issue.documents} 份文件</small>
                    </td>
                    <td>
        ...
        <aside className="panel consistency-detail">
          ...
          <section className="source-comparison">
            <div className="section-title">
              <h3>来源并排对比</h3>
              <span>{selected.sources.length} 个来源值</span>
            </div>
            <div className="source-cards">
              {selected.sources.map((item, index) => (
                <article
                  key={item.id}
                  className={
                    item.value === selected.suggestedValue ? "suggested" : ""
                  }
                >
                  <header>
                    <span>来源 {String.fromCharCode(65 + index)}</span>
                    {item.value === selected.suggestedValue && (
                      <em>
                        <Check size={10} />
                        建议标准
                      </em>
        ...
        {selected.sources
          ...
          .map((item) => (
            <DocumentViewer
              ...
              pageCount={Math.max(item.page, 1)}
              excerpt={item.excerpt}
              focusLabel={`${selected.field}：${item.value}`}
              sourceLocation={`第 ${item.page} 页 · ${item.modifiedAt}`}
              demo
            />
          ))}
      </DocumentDialog>
