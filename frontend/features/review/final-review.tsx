...
import { AlertOctagon, ArrowRight, Bot, CheckCircle2, FileDown } from "lucide-react";
...
export function FinalReview({ projectId, data, agentResult, errors = [] }: { projectId: string; data: ReviewData; agentResult: AgentDataResult<AgentRunBundle>; errors?: string[] }) {
  ...
  return <div className="page">
    ...
    <section className={styles.metrics} aria-label="最终复核概况">
      <Metric href={`/projects/${projectId}/requirements`} label="招标要求" value={data.requirements.length} detail="回到原文逐条复核" />
      <Metric href={`/projects/${projectId}/disqualifications`} label="否决风险" value={unresolvedDisqualifications.length} detail={`${unresolvedDisqualifications.filter((item) => item.status === "confirmed" || item.status === "rule_hit").length} 项已命中规则`} tone="fatal" />
      <Metric href={`/projects/${projectId}/evidence-matching`} label="证据匹配" value={`${matched}/${data.matches.length}`} detail={`${pendingMatches} 个候选待决定`} />
      <Metric href={`/projects/${projectId}/tasks`} label="缺失材料" value={missingMaterials} detail="要求或响应标记缺少材料" tone="warning" />
      <Metric href={`/projects/${projectId}/responses`} label="投标响应" value={data.responses.length} detail={`${data.responses.filter((item) => item.status === "approved").length} 条已批准`} />
      <Metric href={`/projects/${projectId}/tasks`} label="待人工重点检查" value={focus} detail="低置信度、待决定、待复核与待整改" tone="warning" />
    </section>
    ...
    <div className={styles.grid}>
      ...
      <section className="panel"><div className="panel-header"><div><h2>导出文件</h2><p>下载使用现有文件封装工作台，确保封装检查与下载路径一致。</p></div><Link href={`/projects/${projectId}/package`}>全部文件 <ArrowRight size={13} /></Link></div><div className={styles.files}>{artifacts.length > 0 ? artifacts.map((item) => <Link key={item.id} href={item.href}><span><strong>{item.title || "导出产物"}</strong><small>{item.summary || "打开或下载 Agent 生成的产物"}</small></span><ArrowRight size={13} /></Link>) : files.length > 0 ? files.slice(0, 6).map((file) => <Link key={file.id} href={`/projects/${projectId}/package`}><span><strong>{file.name}</strong><small>{file.status === "valid" ? "已纳入封装，打开工作台下载" : "需在封装工作台处理"}</small></span><ArrowRight size={13} /></Link>) : <p className={styles.empty}>暂无可导出文件。请在文件封装工作台生成并校验交付物。</p>}</div></section>
    </div>
  </div>;
}

function Metric({ href, label, value, detail, tone = "" }: { href: string; label: string; value: string | number; detail: string; tone?: string }) { return <Link className={`${styles.metric} ${tone === "fatal" ? styles.fatal : tone === "warning" ? styles.warning : ""}`} href={href}><small>{label}</small><strong>{value}</strong><p>{detail}</p></Link>; }
