...
import { isRemoteApiConfigured } from "@/lib/api/client";
import { projectApi } from "@/lib/api/projects";

const steps = ["基本信息", "招标文件", "项目附件", "补充公告", "企业主体", "材料范围", "确认解析"];
type UploadFile = { name: string; size: number; type: string; status: "ready" | "done"; file: File };
...
export function ProjectWizard() {
  ...
  const remoteMode = isRemoteApiConfigured && !forceDemo;
  const canContinue = useMemo(() => step !== 0 || Boolean(name && code && buyer), [buyer, code, name, step]);

  function addFiles(fileList: FileList | File[]) {
    const incoming = Array.from(fileList).map((file) => ({ name: file.name, size: file.size, type: file.name.split(".").pop()?.toUpperCase() ?? "FILE", status: "done" as const, file }));
  ...
  async function startParsing() {
    ...
    try {
      ...
      setCurrentStep(`上传主招标文件：${files[0].name}`);
      const document = await projectApi.uploadDocument(project.id, files[0].file, "tender_main");
      setProgress(45); setCurrentStep("提交文档解析任务");
      const parseResult = await projectApi.parseDocument(document.id);
      await waitForJob(parseResult.job_id, 45, 65, "后端正在解析文档");
      setProgress(67); setCurrentStep("提取结构化招标要求");
      const extractResult = await projectApi.extractRequirements(project.id, document.id);
    ...
    <div className="page wizard-page">
      ...
      <section className="panel wizard-panel">
        ...
        <div className="wizard-body">
          ...
          {step >= 1 && step <= 3 && <UploadArea files={files} onFiles={addFiles} onDrop={drop} onRemove={(name) => setFiles((current) => current.filter((file) => file.name !== name))} hint={step === 1 ? "上传招标主文件" : step === 2 ? "上传技术、商务及报价附件" : "上传补充公告或澄清文件（可暂时跳过）"} />}
          ...
          {step === 6 && !complete && <div className="review-grid"><Review label="项目" value={`${name} · ${code}`} /><Review label="采购人" value={buyer} /><Review label="企业主体" value="上海智园数字科技有限公司" /><Review label="待处理文件" value={`${files.length} 个已选择文件 · SHA256 由服务端验证`} /><Review label="运行模式" value={remoteMode ? "真实后端 API · 创建 / 上传 / 解析 / 进度轮询" : "本地确定性演示 · 不写入后端"} /><Review label="解析策略" value="Docling 优先 · 扫描页自动 OCR · 低置信度转人工复核" /><div className="safety-note"><FileArchive size={18} /><span><strong>解析不会直接形成最终合规结论</strong><small>所有要求、否决项候选与低置信度结果都保留来源，并进入人工确认。</small></span></div>{(parsing || workflowError) && <div className={workflowError ? "workflow-status error" : "workflow-status"}><strong>{workflowError || currentStep}</strong>{parsing && <div className="workflow-progress"><span style={{ width: `${progress}%` }} /><em>{progress}%</em></div>}{workflowError && remoteMode && <button className="button small" type="button" onClick={() => { setForceDemo(true); setWorkflowError(""); setProgress(0); }}>切换本地演示</button>}</div>}</div>}
