...
const requirements: Requirement[] = [
  {
    ...
    clause: "3.1",
    originalText: "提供营业执照",
    normalizedText: "提供营业执照",
    expectedEvidence: "营业执照",
    actualValue: "待核验",
    rule: "人工复核",
    reasoning: "待确认",
    sourceDocument: "招标文件",
...
const disqualifications: DisqualificationItem[] = [
  {
    id: "dq-1",
    title: "资质缺失",
    status: "rule_hit",
...
const matches: EvidenceMatchGroup[] = [
  {
    id: "match-1",
    requirementCode: "REQ-1",
    requirementTitle: "营业执照",
    risk: "medium",
    requirementStatus: "review",
    page: 4,
    selectedEvidenceIds: ["asset-1"],
    candidates: [
      {
        id: "candidate-1",
        evidenceId: "asset-1",
...
const packageTree: PackageNode[] = [
  {
    ...
    status: "valid",
    children: [
      { id: "file-1", name: "投标响应.docx", type: "file", status: "valid" },
    ],
  },
];
  ...
  it("aggregates existing workbench facts and routes review and export work", () => {
    ...
    render(
      <FinalReview
        ...
        agentResult={{ source: "demo", data: bundle, error: null }}
      />,
    );
    expect(
      screen.getByRole("heading", { name: "最终复核" }),
    ).toBeInTheDocument();
    expect(screen.getByText("4 项仍需人工关注")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /核对要求/ })).toHaveAttribute(
      "href",
      "/projects/p-1/requirements",
    );
    expect(screen.getByRole("link", { name: /处理风险/ })).toHaveAttribute(
      "href",
      "/projects/p-1/disqualifications",
    );
    expect(
      screen.getAllByRole("link", { name: /查看交付包/ })[0],
    ).toHaveAttribute("href", "/projects/p-1/package");
    expect(
      screen.getByText("已生成，等待或已完成人工结论"),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: /查看 Agent/ }),
    ...
    render(
      <FinalReview
        ...
        agentResult={{
          ...
          data: null,
          error: {
            code: "agent_run_request_failed",
            message: "Agent 运行请求失败：API_503。",
            retryable: true,
          },
        }}
        errors={["投标响应：API_503"]}
  ...
  it("only completes the pending final work package review with a non-empty audited reason", async () => {
    ...
    render(
      <FinalReview
        ...
        agentResult={{ source: "api", data: bundle, error: null }}
      />,
    );
    await user.click(screen.getByRole("button", { name: "完成本轮人工复核" }));
    expect(approve).not.toHaveBeenCalled();
    expect(screen.getByRole("alert")).toHaveTextContent("请填写本轮复核说明");
    await user.type(screen.getByLabelText("复核说明"), "已逐项检查工作包。");
    await user.click(screen.getByRole("button", { name: "完成本轮人工复核" }));
    expect(approve).toHaveBeenCalledWith("approval-final", {
      reason: "已逐项检查工作包。",
    });
