...
产品中心是“项目与文件 → 合规矩阵 → 原文与证据 → 响应复核 → 整改 →
交付检查 → 最终人工复核”。每个页面围绕一个明确工作对象组织信息：

- 文件接收同时呈现项目资料、文件用途和逐文件处理状态；
- 合规审阅把要求清单、判断详情与原文页码保持联动；
- 企业材料以主体、有效期、Claim、使用项目和版本支持证据复用；
- 响应编制用项目大纲、状态行和单条原位展开支持连续复核；
- 整改与交付页面直接呈现负责人、截止时间、阻断项和下一步动作。

Agent 运行、审计和本地环境只作为辅助能力，不改变人工复核与最终批准
在业务工作台中的位置。
...
![文件接收、逐文件状态与失败续传](assets/portfolio/project-intake-recovery.jpg)

![项目大纲、连续响应列表与单条原位展开](assets/portfolio/hero-response-workbench.jpg)

![材料列表与复用复核详情](assets/portfolio/final-03-evidence-library.png)

![整改清单、顺序交接与复核详情](assets/portfolio/final-05-remediation-tasks.png)
...
- 固定中文招标项目、合成 PDF/DOCX/XLSX 和可重复生成的验证数据；
- 默认离线 Mock provider，以及可在设置中测试并启用的 DeepSeek 连接；
- 响应版本历史、任意版本比较、人工审批与来源证据；
- macOS Apple Silicon 桌面安装包，内置 FastAPI、Next.js 和 Electron 运行时；
- 单元、API、E2E、production build 与桌面安装烟测。

## 个人贡献与 AI 协作

这是我的个人主导作品。我从问题定义与工作流研究开始，完成了产品规划、核心工作流设计、界面与交互收敛、前后端实现、合成数据、测试验收和作品封装。

我的主要工作包括：

- 把“AI 写投标书”重新定义为证据约束下的正式承诺工作流；
- 规划文件接收、合规审阅、企业材料、标书编制、整改、交付和终审七个阶段；
- 规划任务结构并完成业务场景取舍，保持统一的界面语言；
- 实现 Next.js 前端、FastAPI 后端、领域模型和确定性规则；
- 建立合成文档、固定 oracle、单元/集成/E2E 和独立交付验收；
- 组织产品复审、技术验收、截图证据和最终投递包。

AI 工具用于加速资料整理、方案讨论、代码实现和交叉复审；产品方向、能力边界、工程判断和最终验收由我负责。更完整的边界见 [`AUTHORSHIP.md`](AUTHORSHIP.md)。
