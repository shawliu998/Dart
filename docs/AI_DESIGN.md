...
BidEvidence 采用持久化、受边界约束的 Agent loop，而不是一次性 Prompt 或通用聊天代理：

```text
读取项目事实 → 选择下一项受限工具 → 调用既有领域服务
      ↑                                 ↓
      └──── 审批后基于新事实继续 ← 校验产物与人工门禁
```

- Planner 每轮从数据库中的文档、要求、证据、检查、响应、产物和历史工具事件重新建立上下文。
- 下一动作只能来自封闭工具注册表，并受 scope、最大迭代次数和当前事实约束。
- Run、Step、Event、Artifact 与 Approval 都持久化；失败、取消、重试和恢复不会依赖聊天上下文。
- 工具只调用既有业务服务，Agent 不维护第二套投标状态机。
- 需要业务判断时，运行进入 `waiting_approval`；人工完成复核后再从更新后的项目事实继续。
